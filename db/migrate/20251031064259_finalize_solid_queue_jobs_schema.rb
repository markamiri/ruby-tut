class FinalizeSolidQueueJobsSchema < ActiveRecord::Migration[8.0]
  def change
    # Rename old column if needed
    if column_exists?(:solid_queue_jobs, :job_class) && !column_exists?(:solid_queue_jobs, :class_name)
      rename_column :solid_queue_jobs, :job_class, :class_name
    end

    # Add missing columns safely (won’t duplicate)
